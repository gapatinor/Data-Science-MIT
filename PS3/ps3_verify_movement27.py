...
def test_robot_movement(robot_type, room_type):
    ...
    room = room_type(3, 6, 4)
    if is_furnished:
        room.add_furniture_to_room()
    robots = [robot_type(room, 1, 10)]
    coverage = 0
    time_steps = 0
    min_coverage = 1.0
    if is_furnished:
        anim = ps3_visualize.RobotVisualization(1, 3, 6, room.furniture_tiles) 
    else:
        anim = ps3_visualize.RobotVisualization(1, 3, 6, [])  
